 ...
 */
export function moveElement(layout: Layout, l: LayoutItem, x: ?number, y: ?number, isUserAction: ?boolean, oldDragItem: ?LayoutItem, oldLayout: ?Layout): Layout {
  ...
  if (oldLayout && oldDragItem) {
    for ( let i = 0; i < oldLayout.length; i ++ ) {
      ...
      if ( oldLayout[i].i === oldDragItem.i ) {
        oldPos = i;
      }
    }
  
    oldLayout.splice(oldPos, 1);
    oldLayout.splice(newPos, 0, oldDragItem);
  
    //regenerate the coordinates
    let indexArray = [];
    let len = oldLayout.length;
